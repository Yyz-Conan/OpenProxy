...
public class DataPacketManger {
    ...
    public static byte[] packet(byte[] src) {
        ...
        byte[] packet = new byte[8 + src.length];
        byte[] length = TypeConversion.intToByte(src.length);
        System.arraycopy(length, 0, packet, 0, length.length);
        System.arraycopy(src, 0, packet, length.length, src.length);
        System.arraycopy(PACK_TAG, 0, packet, src.length + length.length, PACK_TAG.length);
        return packet;
    }
