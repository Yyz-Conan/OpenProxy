...
import com.open.proxy.connect.http.client.LocalReceptionProxyClient;
import com.open.proxy.connect.http.client.RemoteReceptionProxyClient;
...
public class MultipleProxyServer extends AbsSecurityServer {
    ...
    protected void onPassChannel(SecurityChannelContext context, SocketChannel channel) {
        ...
        if (mIsServerMode) {
            serverReception = new RemoteReceptionProxyClient(context, channel, getHost());
        } else {
